...
{
    use HasFactory;
    protected $table = "mono_unitermos";
    ...
    static function excluirRegistroByMonografia($id, array $noDelete) {
        return DB::table('mono_unitermos')->where('monografia_id', $id)->whereNotIn('unitermo_id',$noDelete)->delete();
    }
    ...
    public function monografia() {
        return $this->hasMany(Monografia::class, 'mono_unitermos','id','monografia_id');
    }
    ...
    public function unitermos() {
        return $this->hasMany(Unitermos::class, 'mono_unitermos','id','unitermo_id');
    }
}

...
@else
    ...
    <div class="container">
        <div class="row justify-content-start">
          <div class="col-1"></div>  
          <div class="col-10">
            <p>Certifico que o trabalho intitulado <b>{{ $tituloMonografia }}</b>, de autoria do (a) aluno (a) <b>{{ $nomeAluno }}</b> 
            e de seu / sua orientador (a), Professor (a) Doutor (a) <b>{{ $nomeOrientador }}</b>, 
            @if (count($orientadores) > 0) co-orientador(es) 
                @foreach ($orientadores as $do) 
                    {{ $do->nome }},
                @endforeach
            @endif
            foi apresentado na <b>{{ $mostra }}</b> 
            Mostra de Trabalhos de Conclusão de Curso da EEUSP, realizada em {{ $mesMostra }} de <b>{{ $ano }}</b>, sob coordenação da Comissão de Coordenação de 
                 Curso de Bacharelado da Escola de Enfermagem da Universidade de São Paulo.
            </p><br><br>
          </div>
          <div class="col-1"></div>
        </div>

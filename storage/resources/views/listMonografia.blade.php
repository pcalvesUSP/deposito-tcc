...
<table class="tableData" id="listMonografias" border="1">
    ...
    <form id="filtrarMonografia" action="{{ route('busca.monografia') }}" method="post">
      ...
      <tr>
        <th @if ($userLogado == "Orientador") colspan="7" @else colspan="5" @endif style="text-align:right;background:#c6c2eb;">Filtrar: <input type="text" id="filtro" name="filtro" value="{{ empty($filtro)?old('filtro'):$filtro }}" size="15" class="campo" style="font-weight:bold;width:150px;"/></th>
      </tr>
    </form>
    <tr>
